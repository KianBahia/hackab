function ThemeToggle({ isDarkMode, setIsDarkMode }) {
  ...
  return (
    ...
    >
      {/* Toggle circle */}
      <div
        className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-all duration-300 flex items-center justify-center ${
          isDarkMode ? "translate-x-0" : "translate-x-8"
        }`}
      >
        {/* Moon icon - shown when in dark mode (left side) */}
        {isDarkMode && (
          <svg
        ...
        {/* Sun icon - shown when in light mode (right side) */}
        {!isDarkMode && (
          <svg
            className="w-4 h-4 text-black"
            fill="currentColor"
